...
tf::Vector3  _normalize3 (const tf::Vector3 & v0);
double         _dist3 (const tf::Vector3 & v0, const tf::Vector3 & v1);
...
{
        os << "Vector = " << boost::format("%11.4f") % v.getX() << ":"
           << boost::format("%11.4f") % v.getY() << ":"
           << boost::format("%11.4f") % v.getZ() << std::endl;
	return os;
}
...
inline tf::Vector3 _normalize3 (const tf::Vector3 & v0)
{
        tf::Vector3 v(v0);
        return v.normalized();
#if 0
...
inline tf::Quaternion _quatFromMatrix (RosMatrix & mat)
{
	...
	{
                rot.setW( tr + 1);
                rot.setX ( mat[4 * 2 + 1] - mat[4 * 1 + 2] );
                rot.setY( mat[4 * 0 + 2] - mat[4 * 2 + 0] );
                rot.setZ( mat[4 * 1 + 0] - mat[4 * 0 + 1] );
	}
	else
	{
		...
		if ( mat[4 * 1 + 1] > mat[4 * 0 + 0] )
		{
			...
			{
                                rot.setW (( mat[4 * 2 + 2] - ( mat[4 * 0 + 0] + mat[4 * 1 + 1] ) ) + 1);
                                rot.setX ( mat[4 * 2 + 0] + mat[4 * 0 + 2] );
                                rot.setY ( mat[4 * 1 + 2] + mat[4 * 2 + 1] );
                                rot.setZ ( mat[4 * 1 + 0] - mat[4 * 0 + 1] );
			}
			else
			{
                                rot.setY  (( mat[4 * 1 + 1] - ( mat[4 * 2 + 2] + mat[4 * 0 + 0] ) ) + 1);
                                rot.setZ ( mat[4 * 1 + 2] + mat[4 * 2 + 1] );
                                rot.setX  ( mat[4 * 0 + 1] + mat[4 * 1 + 0] );
                                rot.setW  ( mat[4 * 0 + 2] - mat[4 * 2 + 0] );
			}
		}
		else if ( mat[4 * 2 + 2] > mat[4 * 0 + 0] )
		{
                        rot.setZ (( mat[4 * 2 + 2] - ( mat[4 * 0 + 0] + mat[4 * 1 + 1] ) ) + 1);
                        rot.setX ( mat[4 * 2 + 0] + mat[4 * 0 + 2] );
                        rot.setY  ( mat[4 * 1 + 2] + mat[4 * 2 + 1] );
                        rot.setW  ( mat[4 * 1 + 0] - mat[4 * 0 + 1] );
		}
		else
		{
                        rot.setX (( mat[4 * 0 + 0] - ( mat[4 * 1 + 1] + mat[4 * 2 + 2] ) ) + 1);
                        rot.setY ( mat[4 * 0 + 1] + mat[4 * 1 + 0] );
                        rot.setZ ( mat[4 * 2 + 0] + mat[4 * 0 + 2] );
                        rot.setW ( mat[4 * 2 + 1] - mat[4 * 1 + 2] );
		}
	}
        //double fnorm = std::sqrt(rot.x() * rot.x() + rot.y() * rot.y() + rot.z() * rot.z() + rot.w() * rot.w);
        double fnorm = rot.length();
...
inline tf::Vector3 _quatToAxisAngle(const tf::Quaternion q)
{
  ...
  if (SMALL(sh)) {
    ...
    r.m_floats[0]  = mag * q.x();
    r.m_floats[1] = mag * q.y();
    r.m_floats[2] = mag * q.z();
  }
...
inline tf::Pose _poseFromMatrix (const boost::array<double, 12> & m)
{
        ...
        t.setRotation(_quatFromMatrix( ( RosMatrix & )m));
...
{
        boost::array<double, 12> m = _matrixFromQuat(t.getRotation());
        m[3]  = t.getOrigin().x();
        m[7]  = t.getOrigin().y();
        m[11] = t.getOrigin().z();
        return m;
...
inline tf::Vector3 _vectorMatrixMult (const RosMatrix & m, const tf::Vector3 & v)
{
        ...
        vnew.m_floats[0] = m[4 * 0 + 0] * v.x() + m[4 * 0 + 1] * v.y() + m[4 * 0 + 2] * v.z() + m[4 * 0 + 3];
        vnew.m_floats[1] = m[4 * 1 + 0] * v.x() + m[4 * 1 + 1] * v.y() + m[4 * 1 + 2] * v.z() + m[4 * 1 + 3];
        vnew.m_floats[2] = m[4 * 2 + 0] * v.x() + m[4 * 2 + 1] * v.y() + m[4 * 2 + 2] * v.z() + m[4 * 2 + 3];
	return vnew;
}
...
inline tf::Vector3 _quatCartMult (const tf::Quaternion q1, const tf::Vector3 v2)
{
	...
	if ( !_quatIsNorm(q1) )
	{
        ...
        c.m_floats[0] = q1.y() * v2.z() - q1.z() * v2.y();
        c.m_floats[1] = q1.z() * v2.x() - q1.x() * v2.z();
        c.m_floats[2] = q1.x() * v2.y() - q1.y() * v2.x();

        vout.m_floats[0] = v2.x() + 2 * ( q1.w() * c.x() + q1.y() * c.z() - q1.z() * c.y() );
        vout.m_floats[1] = v2.y() + 2 * ( q1.w() * c.y() + q1.z() * c.x() - q1.x() * c.z() );
        vout.m_floats[2] = v2.z() + 2 * ( q1.w() * c.z() + q1.x() * c.y() - q1.y() * c.x() );
...
inline tf::Quaternion _quatSlerp(tf::Quaternion  & qa, tf::Quaternion & qb, double t) {
	...
	if (fabs(theta) < 0.01){
            qm=qa;
               //qm.setW(qa.w); qm.x() = qa.x;  qm.y() = qa.y; qm.z() = qa.z;
		return qm;
	}
	...
	if (fabs(sinTheta) < 0.01){
                qm.setW(qa.w() * 0.5 + qb.w() * 0.5);
                qm.setX(qa.x() * 0.5 + qb.x() * 0.5);
                qm.setY(qa.y() * 0.5 + qb.y() * 0.5);
                qm.setZ (qa.z() * 0.5 + qb.z() * 0.5);
		return qm;
	}
	...
	//calculate Quaternion.
        qm.setW (qa.w() * ratioA + qb.w() * ratioB);
        qm.setX (qa.x() * ratioA + qb.x() * ratioB);
        qm.setY (qa.y() * ratioA + qb.y() * ratioB);
        qm.setZ ( qa.z() * ratioA + qb.z() * ratioB);
	return qm;
}
...
inline tf::Quaternion _quatMult (const tf::Quaternion & quat0, const tf::Quaternion & quat1)
{
        ...
        q.setX( quat0.w() * quat1.x() + quat0.x() * quat1.w() + quat0.y() * quat1.z() - quat0.z() * quat1.y());
        q.setY( quat0.w() * quat1.y() + quat0.y() * quat1.w() + quat0.z() * quat1.x() - quat0.x() * quat1.z());
        q.setZ( quat0.w() * quat1.z() + quat0.z() * quat1.w() + quat0.x() * quat1.y() - quat0.y() * quat1.x());
        q.setW( quat0.w() * quat1.w() - quat0.x() * quat1.x() - quat0.y() * quat1.y() - quat0.z() * quat1.z());
...
inline tf::Quaternion _quatFromAxisAngle (const tf::Vector3 & r)
{
  ...
  sincos(0.5 * mag, (double *)  &sh, (double *)  &( q.w() ) );
  ...
  const tf::Vector3 & v2)
{
  ...
  if ( SMALL(_magnitude3(cross) ) )
  {
    ...
    u2     = _normal3(u1);
    rvec.m_floats[0] = M_PI * u2.x();
    rvec.m_floats[1] = M_PI * u2.y();
    rvec.m_floats[2] = M_PI * u2.z();
  ...
  rvec.m_floats[0] = th * cross.x();
  rvec.m_floats[1] = th * cross.y();
  rvec.m_floats[2] = th * cross.z();
  quat   = _quatFromAxisAngle(rvec);
  return quat;
}
//----------------------------------------------------------------------------
inline tf::Pose _poseMult (const tf::Pose & p0, const tf::Pose & p1)
{
        ...
        p.setOrigin( _poseMult(p0, p1.getOrigin()));
        p.setRotation(_quatMult(p0.getRotation(), p1.getRotation()));
	return p;
}
inline tf::Pose _poseInverse (const tf::Pose & p)
{
        ...
        pinv.getRotation().setX( -p.getRotation().x());
        pinv.getRotation().setY( -p.getRotation().y());
        pinv.getRotation().setZ( -p.getRotation().z());
        pinv.getRotation().setW( p.getRotation().w());
        tf::Vector3 t = _poseMult(pinv, p.getOrigin());
        pinv.getOrigin().x() = -t.x();
        pinv.getOrigin().y() = -t.y();
        pinv.getOrigin().z() = -t.z();
#endif
        return pinv.inverse();
}
inline tf::Vector3 _poseMult (const tf::Pose & p, const tf::Vector3 & v)
{
        double        ww = 2 * p.getRotation().x() * p.getRotation().x();
        double        wx = 2 * p.getRotation().x() * p.getRotation().y();
        double        wy = 2 * p.getRotation().x() * p.getRotation().z();
        double        wz = 2 * p.getRotation().x() * p.getRotation().w();
        double        xx = 2 * p.getRotation().y() * p.getRotation().y();
        double        xy = 2 * p.getRotation().y() * p.getRotation().z();
        double        xz = 2 * p.getRotation().y() * p.getRotation().w();
        double        yy = 2 * p.getRotation().z() * p.getRotation().z();
        double        yz = 2 * p.getRotation().z() * p.getRotation().w();
        tf::Vector3 vnew;

        vnew.m_floats[0] = ( 1 - xx - yy ) * v.x() + ( wx - yz ) * v.y() + ( wy + xz ) * v.z() + p.getOrigin().x();
        vnew.m_floats[1] = ( wx + yz ) * v.x() + ( 1 - ww - yy ) * v.y() + ( xy - wz ) * v.z() + p.getOrigin().y();
        vnew.m_floats[2]= ( wy - xz ) * v.x() + ( xy + wz ) * v.y() + ( 1 - ww - xx ) * v.z() + p.getOrigin().z();
	return vnew;
}
inline tf::Pose _poseInv (const tf::Pose & p1)
{
        ...
        p2.setRotation( _quatInv(p1.getRotation()));

        p2.setOrigin(_quatCartMult(p2.getRotation(), p1.getOrigin()));

        p2.getOrigin().m_floats[0] = -p2.getOrigin().x();
        p2.getOrigin().m_floats[1] = -p2.getOrigin().y();
        p2.getOrigin().m_floats[2] = -p2.getOrigin().z();
	return p2;
}
