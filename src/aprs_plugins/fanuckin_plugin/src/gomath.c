...
#include "config.h"
#endif
#include "fanuckin_plugin/gotypes.h"		/* go_result,real, etc */
#include "fanuckin_plugin/gomath.h"		/* these decls, go_cart, etc. */
...
/* give sincos() to users as go_sincos() */
...
{
  if (s < -1.0 || s > 1.0) return GO_RESULT_DOMAIN_ERROR;
...
{
  if (c < -1.0 || c > 1.0) return GO_RESULT_DOMAIN_ERROR;
...
/*
  If the local implementation of cbrt() below is inadequate due to the
  ldexp, frexp calls that may not exist, use pow(x,1/3) instead. Define
  CBRT_IS_POW in this case.
*/
#define CBRT_IS_POW

#if defined(CBRT_IS_POW)
...
go_result go_sph_cart_convert(const go_sph *s, go_cart *v)
{
  ...
  go_sincos(s->theta, &sth, &cth);
  go_sincos(s->phi, &sph, &cph);
...
go_result go_uxz_mat_convert(const go_uxz *uxz, go_mat *mat)
{
  ...
  retval = go_cart_unit(&uxz->x, &ux);
  if (GO_RESULT_OK != retval) return retval;
  retval = go_cart_cart_proj(&uxz->z, &ux, &uz);
  if (GO_RESULT_OK != retval) return retval;
  go_cart_cart_sub(&uxz->z, &uz, &uz);
  retval = go_cart_unit(&uz, &uz);
  if (GO_RESULT_OK != retval) return retval;
  mat->x = ux;
  mat->z = uz;
  go_cart_cart_cross(&uz, &ux, &mat->y);
...
go_complex go_complex_inv(go_complex z, go_result * result)
{
  ...
  denom = go_sq(z.re) + go_sq(z.im);
  if (denom < GO_REAL_EPSILON) {
    if (NULL != result) *result = GO_RESULT_DIV_ERROR;
    return zout;
  }

  zout.re = z.re, zout.im = -z.im;
  if (NULL != result) *result = GO_RESULT_OK;
  return go_complex_scale(zout, 1.0 / denom);
}
			   ...
			   go_complex * z4)
{
  ...
  go_real a2, a3, a4;
  go_real e, f, g;
  go_real mag1, mag2, mag3;
  go_complex p, q, r;
  go_complex biga, bigb;
  go_result retval;
  ...
  */
  ...
  /* pick the two biggest roots */
  mag1 = go_complex_magsq(*z1);
  mag2 = go_complex_magsq(*z2);
  mag3 = go_complex_magsq(*z3);
  if (mag1 > mag2) {
    biga = *z1;
    if (mag2 > mag3) {
      bigb = *z2;
  ...
  go_complex_sqrt(biga, &p, NULL);
  go_complex_sqrt(bigb, &q, NULL);
  r = go_complex_inv(go_complex_scale(go_complex_mult(p, q), -8.0/f), &retval);
  if (GO_RESULT_OK != retval) return retval;

  a4 = 0.25 * quart->a;
  *z1 = go_complex_add(go_complex_add(p, q), r);
  z1->re -= a4;
  *z2 = go_complex_sub(go_complex_sub(p, q), r);
  z2->re -= a4;
  *z3 = go_complex_sub(go_complex_sub(q, p), r);
  z3->re -= a4;
  *z4 = go_complex_sub(go_complex_sub(r, p), q);
  z4->re -= a4;
...
*/
go_result go_cart_cart_pose(const go_cart *v1,
			    const go_cart *v2,
			    go_cart *v1c,
			    go_cart *v2c,
			    go_integer num,
			    go_pose *pout)
{
  ...
  pout->rot.y = N.el[2][t];
  pout->rot.z = N.el[3][t];
  retval = go_quat_norm(&pout->rot, &pout->rot);
  if (GO_RESULT_OK != retval) return retval;

  /* rotate left centroid, subtract left from right to get translation */
  (void) go_quat_cart_mult(&pout->rot, &c1, &c1);
  (void) go_cart_cart_sub(&c2, &c1, &pout->tran);
...
go_result go_rpy_cart_mult(const go_rpy *rpy, const go_cart *in, go_cart *out)
{
  ...
  return go_quat_cart_mult(&quat, in, out);
}
...
go_flag go_quat_quat_compare(const go_quat *q1, const go_quat *q2)
{
  ...
  /* note anti-parallel (0, x, y, z) = (0, -x, -y, -z) */
  if (GO_ROT_SMALL(q1->s) && GO_ROT_SMALL(q2->s)) {
    if (GO_TRAN_CLOSE(q1->x, q2->x) &&
	GO_TRAN_CLOSE(q1->y, q2->y) &&
	GO_TRAN_CLOSE(q1->z, q2->z)) {
      return 1;
    }
...
/*
  go_pose_pose_mult() implements compound transforms,

  0    0    1
  .T =  T *  T
  2    1    2

  Expressed in terms of individual position vectors P and rotations R,
  ...
  This would be called as go_pose_pose_mult(&T_1_0, &T_2_1, &T_2_0).
*/
...
go_result go_hom_hom_mult(const go_hom *h1, const go_hom *h2, go_hom *hout)
{
  ...
  retval = go_mat_cart_mult(&h1->rot, &h2->tran, &out.tran);
  if (GO_RESULT_OK != retval) return retval;

  retval = go_cart_cart_add(&h1->tran, &out.tran, &out.tran);
  if (GO_RESULT_OK != retval) return retval;

  retval = go_mat_mat_mult(&h1->rot, &h2->rot, &out.rot);
...
go_result go_pose_vel_mult(const go_pose *pose, const go_vel *vel, go_vel *out)
{
  ...
  */
  ...
  go_cart_cart_cross(&vel->w, &poseinv.tran, &wxp);
  go_cart_cart_add(&vel->v, &wxp, &v);
  go_quat_cart_mult(&pose->rot, &v, &out->v);
  go_quat_cart_mult(&pose->rot, &vel->w, &out->w);
...
extern go_result go_mat3_inv(go_real a[3][3],
			     go_real ainv[3][3])
{
  go_real a11a22;
  go_real a11a23;
...
extern go_result go_mat4_inv(go_real a[4][4],
			     go_real ainv[4][4])
{
  go_real work[4][4];
  go_real denom;
...
go_result go_dh_hom_convert(const go_dh * dh, go_hom * h)
{
  ...
  go_sincos(dh->theta, &sth, &cth);
  go_sincos(dh->alpha, &sal, &cal);
...
go_result go_pose_dh_convert(const go_pose *p, go_dh * dh)
{
  ...
  (void) go_pose_hom_convert(p, &h);
  return go_hom_dh_convert(&h, dh);
}
...
go_result go_link_joint_set(const go_link * link, go_real joint, go_link * linkout)
{
  ...
  linkout->body.mass = link->body.mass;
  for (i = 0; i < 3; i++) 
    for (j = 0; j < 3; j++)
      linkout->body.inertia[i][j] = link->body.inertia[i][j];
...
/*
  Solve a cos theta + b sin theta = 1 for theta, two solutions, using
  the identities

  cos theta = (1 - tan^2(theta/2)) / (1 + tan^2(theta/2))
  sin theta = 2 * tan(theta/2) / (1 + tan^2(theta/2))

  First normalize the equation so that the RHS is 1. Let T = tan(theta/2).
  Then, 

  (-a-1) T^2 + 2b T + (a-1) = 0

  For a = -1, it degenerates, with solution

  T = Pi, atan2(2b/(b^2+1), (b^2-1)/(b^2+1))

  Otherwise it's quadratic; solve for both T solutions and finish
  by taking theta = 2 arctan(T). No real solutions if a^2 + b^2 < 1.
*/
