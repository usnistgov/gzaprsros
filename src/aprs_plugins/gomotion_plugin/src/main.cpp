...
#define CLOSE_AND_RETURN(ret)			\
  ...
  if (6 != sscanf(inistring, "%lf %lf %lf %lf %lf %lf",
    ...
    CLOSE_AND_RETgomotion_pluginURN(1);
  }
...
int test(void)
{
  ...
  if (GO_RESULT_OK != go_init()) {
    fprintf(stderr, "can't init gomotion\n");
    return 1;
  }

  if (GO_RESULT_OK != genser_kin_init(&kins)) {
    fprintf(stderr, "can't init general serial kinematics\n");
    return 1;
  }
  ...
  /* run the forwards on simple joints */
  printf("foward kins: \n");
  ...
  res = genser_kin_fwd(&kins, joints, &pose);
  if (GO_RESULT_OK != res) {
  ...
  /* run the forwards on lab joints */
  printf("foward kins: \n");
  ...
  /* run the inverse on that */
  printf("inverse kins: \n");

  res = genser_kin_inv(&kins, &pose, joints);
  if (GO_RESULT_OK != res) {
    fprintf(stderr, "%s\n", go_result_to_string(res));
  }
  ...
  /* perturb the Cartesian pose */
  printf("perturbed inverse kins: \n");
  pose.tran.x += 0.1;
  pose.tran.y += 0.1;
  pose.tran.z += 0.1;
  rpy.r += 0.1;
