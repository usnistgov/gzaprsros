...
////////////////////////////////////////////////////////////////////////////////
Crcl::PoseType Convert(tf::Pose pose, double lengthConversion) {
    Crcl::PoseType crclPose(NullPose());
    crclPose.Point() = GetPoint(pose.getOrigin());
#if 1
    crclPose.Point().X() *= lengthConversion;
    crclPose.Point().Y() *= lengthConversion;
    crclPose.Point().Z() *= lengthConversion;
#endif
...
////////////////////////////////////////////////////////////////////////////////
ActuateJointsType::ActuateJoint_sequence  ConvertCmdJnts(sensor_msgs::JointState joints, double _angleConversion)
{
    ActuateJointsType::ActuateJoint_sequence jout;
    for (unsigned int i = 0; i < joints.position.size(); i++) {
        JointDetailsType details;
        ActuateJointsType::ActuateJoint_type jointcmd(i + 1, joints.position[i] * _angleConversion, details);
        jout.push_back(jointcmd);
    }
    return jout;
}
