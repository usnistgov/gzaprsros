...
////////////////////////////////////////////////////////////////////////////////
std::string CrclClientCmdInterface::ActuateJoints (ActuateJointsType::ActuateJoint_sequence & joints)
{
    ActuateJointsType cmd(++_commandnum);
    cmd.ActuateJoint (joints);
    xml_schema::namespace_infomap map;
    std::ostringstream strfs;
...
#endif
    DwellType cmd(++_commandnum, time);
    xml_schema::namespace_infomap map;
    CRCLCommandInstance(strfs, // std::cout,
            (CRCLCommandType &) cmd,
            map,
...
CrclReturn CrclStatusMsgInterface::ParseCRCLStatusString(std::string str) {
    ...
    try {
        ...
        //
        if(crclstat->JointStatuses().present())
            VALIDSTORE(_status._CurrentJoints, crclstat->JointStatuses().get().JointStatus());
