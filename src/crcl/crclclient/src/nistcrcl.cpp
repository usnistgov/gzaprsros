...
/// crclClient
////////////////////////////////////////////////////////////////////////////////
bool crclClient::bDebugStatusMsg=false;
bool crclClient::bDebugCommandMsg=false;
bool crclClient::bCrclStopIgnore=false;
bool crclClient::bFlywheel=false;
bool crclClient::bProcessAllCrclMessages=false;
std::string crclClient::sRobot;
std::ofstream *crclClient::debugstream=(std::ofstream *) &nullout;



////////////////////////////////////////////////////////////////////////////////
crclClient::crclClient()
{

}
int crclClient::init(std::string crclIp,
                       int crclport,
                       double d_cycle_time,
                       ...
                       )
{
    ...
    // Create loop to send rcs->crcl commands and receive crcl->rcs status
    rcs2crcl=std::shared_ptr<CRcs2Crcl> (new CRcs2Crcl());

    // Initialize subelements of crcl2ros
    rcs2crcl->init(crclIp, crclport, d_cycle_time,robotkin);

    CCrclBufferHandler::_bTrace = false;
    ...
    std::string logFolder=CrclLogger.loggerFolder();
    CrclLogger.open(logFolder+"/crclxml_" + rcs2crcl->_robotkin.robot_name + ".log");
    CrclLogger.debugLevel()=3;
    CrclLogger.isTimestamping( )=true;
    CrclLogger.isOutputConsole()=true;
...
//    std::string angle_units = cfg.getSymbolValue<std::string>("CRCL.angle_units", "RADIAN");
    ...
    crclClient::bDebugStatusMsg = (bool) cfg.getSymbolValue<int>("CRCL.DebugStatusMsg", "0");
    crclServer::bDebugCommandMsg = (bool) cfg.getSymbolValue<int>("CRCL.DebugCommandMsg", "0");
    crclClient::bCrclStopIgnore = (bool) cfg.getSymbolValue<int>("CRCL.StopIgnore", "0");
    CCrclSession::bDebugXML = (bool) cfg.getSymbolValue<int>("CRCL.DebugXML", "0");
    crclClient::bFlywheel=(bool) cfg.getSymbolValue<double>("CRCL.flywheel", "0");
    crclClient::bProcessAllCrclMessages=(bool) cfg.getSymbolValue<double>("CRCL.processAllCrclMessages", "0");
    crclClient::sRobot= cfg.getSymbolValue<std::string>("system.robots","robie");
#endif
...
////////////////////////////////////////////////////////////////////////////////
void crclClient::start ( )
{
    // Start rcs translater to crcl msg
    // which starts the socket client
    logStatus( " rcs2crcl started %s\n", getTimeStamp(GMT_UV_SEC).c_str());
    rcs2crcl->start();

}

////////////////////////////////////////////////////////////////////////////////
void crclClient::stop ( )
{
    logStatus( "Stop crclClient %s\n", getTimeStamp(GMT_UV_SEC).c_str());
...
////////////////////////////////////////////////////////////////////////////////
int crclClient::addcommand(crcl_rosmsgs::CrclCommandMsg cmd)
{
    return rcs2crcl->addcommand(cmd);
}

////////////////////////////////////////////////////////////////////////////////
int crclClient::cmd_status(int cmdnum)
{
    return rcs2crcl->cmd_status(cmdnum);
}

////////////////////////////////////////////////////////////////////////////////
bool crclClient::isDone()
{
    RCS::CCanonWorldModel state;
    syncGetStatus(state); // request latest copy as it may not be latest

    if((rcs2crcl->QSize() == 0)
            && state.crclCommandStatus!=RCS::CanonStatusType::CANON_WORKING
            )
        return true;
    return false;
}
...
////////////////////////////////////////////////////////////////////////////////
int crclClient::syncGetStatus(RCS::CCanonWorldModel & status)
{
    return rcs2crcl->getStatus(status);

}

////////////////////////////////////////////////////////////////////////////////
bool crclClient::isConnected()
{
    return rcs2crcl->session->IsConnected();
}
