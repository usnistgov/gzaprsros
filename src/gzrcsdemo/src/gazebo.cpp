...
std::shared_ptr<CGazebo> gz;
gazebo::transport::NodePtr _node= gazebo::transport::NodePtr(new gazebo::transport::Node());
...
{
    // this is here or a checked delete exception is thrown by boost :(
    _node=nullptr;
...
{
    _sub = _node->Subscribe<gazebo::msgs::Model>(_modeltopicname, &CGzModelReader::onUpdate,this);
    Globals.sleep(1000);
