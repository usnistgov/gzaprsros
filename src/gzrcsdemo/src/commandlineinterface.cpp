...
////////////////////////////////////////////////////////////////////////////////
CComandLineInterface::CComandLineInterface(CGearDemo &_geardemo): geardemo(_geardemo)
{
    _bDegrees = false;
    _bFlag = true;
    _ncindex=0;
...
bool CComandLineInterface::jog(char c,double amt, int &jnt, int &axis)
{
    ...
    if(jnt>=0)
    {
        ...
        geardemo.crclApi->moveJoints(Globals.allJointNumbers(), nextpos.position);
    }
    ...
    if(axis>=0)
    {
        ...
        try{
            ncs[_ncindex]->robotKinematics()->IK(r_curpose,
                                           ...
                                           joints.position);
            geardemo.crclApi->moveJoints(ncs[_ncindex]->robotKinematics()->allJointNumbers(), joints.position);
        }
        catch(...)
        {
...
int CComandLineInterface::inputLoop()
{
    ...
    struct pollfd fds;
    int ret;
    bool bConnected = geardemo.crclApi->isConnected();
    if(!bConnected)
    {
        std::cout<<"Crcl socket not connected\n";
    }

    //RCS::CCanonWorldModel wm= geardemo.crclApi->getStatus();
...
int CComandLineInterface::interpretMacro(std::string macro_name)
{
    ...
    if((it=Globals.namedCommand().find(macro_name))!= Globals.namedCommand().end())
...
int CComandLineInterface::interpretLine(std::string line)
{
    ...
    else if (msg.compare( 0, strlen("macro "), "macro ") == 0 )
    {
        ...
        std::map<std::string, std::vector<std::string>>::iterator it;
        if((it=Globals.namedCommand().find(msg))== Globals.namedCommand().end())
            return RCS::NORMAL;
...
//        ncs[_ncindex]->setGripperJointSpeeds(ncs[_ncindex]->currentGripperJointSpeed()*.5);
    ...
    else if (msg.compare( 0, strlen("status"),"status") == 0)
    {
       ...
       RCS::CCanonWorldModel wm;
       if(geardemo.crclApi->getStatus(wm))
       {
           std::cout<<"Problem reading crcl status\n";
           return RCS::NOOP;
       }
       std::stringstream ss;
       std::string c = (bConnected)? "true\n" : "false\n";
       ss << "Connected  =      " <<  c;
       ss << "Status  =         " << CanonStatusType::value(wm.crclCommandStatus) << "\n";
       ss << "Command =         " << sCmd(wm.echoCmdId) << "\n";
       ss << "Current Pose =    " << RCS::dumpPoseSimple(wm.currentpose) << "\n";
       ss << "Current Joints =  " << RCS::vectorDump<double>(wm.robotjoints.position) << "\n";
       std::cout << ss.str();
    ...
    {
        std::cout << "reset controller\n";
    }

    else if (msg.compare("speeds") == 0)
    {
        std::cout << "speed:\n\trobot joint vel=" << Globals.currentRobotJointSpeed()
                  << "speed:\n\tjgripper    vel=" <<   Globals.currentGripperJointSpeed()
                  << "\n\tlinear            vel="  <<  Globals.currentLinearSpeed()
                  << "\n\trotational vel=       "  <<   Globals.currentAngularSpeed()
                  << "\n" << std::flush;
    }
#ifdef FIXME
    else if (msg.compare("slower") == 0)
    {
        geardemo.crclApi->slow();
    ...
    else if (msg.compare( 0, strlen("goto "), "goto ") == 0)
    {
        ...
        {
            geardemo.crclApi->moveTo(ncs[_ncindex]->namedPoseMove()[msg]);

        }
        else if(ShapeModel::instances.findInstance(msg)!=NULL)
        {
            geardemo.crclApi->moveTo(msg);
        }        // see if existing joint move name
        else if(ncs[_ncindex]->namedJointMove().find(msg)!= ncs[_ncindex]->namedJointMove().end())
        {
            geardemo.crclApi->moveJoints(ncs[_ncindex]->robotKinematics()->allJointNumbers(), ncs[_ncindex]->namedJointMove()[msg]);
        }
        else
        {
    ...
    {
        geardemo.crclApi->moveJoints(Globals.allJointNumbers(), Globals.namedJointMove()["home"]);
    }
//    else if (msg.compare("safe") == 0)
//    {
//            geardemo.crclApi->move_joints(ncs[ncindex]->robotKinematics()->allJointNumbers(), ncs[ncindex]->NamedJointMove["safe"]);
//    }
    ...
    else if (msg.compare( 0, strlen("joints "), "joints ") == 0 )
    {
        ...
        geardemo.crclApi->moveJoints(Globals.allJointNumbers(), positions);
    }
    else if (msg.compare( 0, strlen("jog "), "jog ") == 0 )
    {
        ...
        // make sure client crcl connected to get current joint values
        bool bConnected = geardemo.crclApi->isConnected();
        if(!bConnected)
        {
            std::cout<<"Crcl socket not connected\n";
        ...
        if(geardemo.crclApi->getStatus(wm))
        {
            std::cout<< "Problem reading status\n";
            return RCS::NORMAL;
        }
        curpos.position = wm.robotjoints.position;
        nextpos.position = wm.robotjoints.position;
        ...
        while(doFlag)
        {
            ...
            //geardemo.crclApi->move_joints(jointnum,positions);
    ...
    {
        geardemo.crclApi->openGripper();
    }
    else if (msg.compare("close") == 0)
    {
       geardemo.crclApi->closeGripper();
    }
    ...
    else if (msg.compare( 0, strlen("retract "), "retract ") == 0 )
    {
        ...
        geardemo.crclApi->retract(msg);
    }
    else if (msg.compare( 0, strlen("retract"), "retract") == 0 )
    {
        geardemo.crclApi->retract();
    }
