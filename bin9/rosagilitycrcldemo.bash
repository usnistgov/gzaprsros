...
cmd+=( --tab  --working-directory="$r" -e 'bash -c "printf \"\e]2;fanuc crcl demo\a\";sleep 20s;export LD_LIBRARY_PATH=/opt/ros/kinetic/lib:$libs:$qtlibs:$LD_LIBRARY_PATH; echo $LD_LIBRARY_PATH; source /opt/ros/kinetic/setup.bash; cd $gzrcsdemo; ./gzrcsdemo -r fanuc_;exec bash"')

cmd+=( --tab  --working-directory="$r" -e 'bash -c "printf \"\e]2;motoman crcl demo\a\";sleep 20s;export LD_LIBRARY_PATH=/opt/ros/kinetic/lib:$libs:$qtlibs:$LD_LIBRARY_PATH; echo $LD_LIBRARY_PATH; source /opt/ros/kinetic/setup.bash; cd $gzrcsdemo; ./gzrcsdemo -r motoman_;exec bash"')
